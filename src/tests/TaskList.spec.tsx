...
describe('<TaskList />', () => {
  it('should be empty', () => {
    const { container, unmount } = render(() => <TaskList tasks={[]} editTaskCall={() => []} isFiltered={false}/>);
  ...
  it('should contain 2 dayGroup', () => {
    ...
    },
    {
      ...
      name: "Test Task 2",
      color: "#00ffff",
      start_date: Date.now() - 360000000,
      end_date: Date.now() - 360000000,
      description: "Test Description 2"
    }];

    const { container, unmount } = render(() => <TaskList tasks={givenTasks} editTaskCall={() => []} isFiltered={false}/>);
  ...
  it('should open todays dayGroup', () => {
    ...
    const { container, unmount } = render(() => <TaskList tasks={givenTasks} editTaskCall={() => []} isFiltered={false}/>);
    expect(container.getElementsByClassName("collapse-open")[0]).not.toBeNull();
    unmount();
  });
  ...
  it('should open all dayGroups when filtered', () => {
    ...
    },
    {
      ...
      name: "Test Task 3",
      color: "#00ffff",
      start_date: Date.now() - 360000000 * 2,
      end_date: Date.now() - 360000000 * 2,
      description: "Test Description 3"
    }];

    const { container, unmount } = render(() => <TaskList tasks={givenTasks} editTaskCall={() => []} isFiltered={true}/>);
    expect(container.getElementsByClassName("collapse-open").length).toBe(3);
    unmount();
  });
  ...
  it('should call editTaskCall when edit button is clicked', () => {
    ...
    const { container, unmount } = render(() => <TaskList tasks={givenTasks} editTaskCall={mock} isFiltered={false}/>);
